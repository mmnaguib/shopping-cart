...
import { nextSlide, prevSlide, dotSlide } from "../app/Slices/SliderSlice";
import { sliderData } from "../assets/data/dummyData";

const Slider = () => {
  const slideIndex = useSelector((state) => state.slider.value);
  const SliderImgs = sliderData.map((item) => {
    return (
      <div
        key={item.id}
        className={
      ...
      >
        <div>
          {parseInt(item.id) === slideIndex && (
            <img className="h-[850px] w-full" src={item.img} alt="shoes"></img>
          )}
        </div>
